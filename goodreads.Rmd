...
get_viaf_number <- function(one_url, .verbose = TRUE){
  # Need to check if wi # is NA. If so, return NA value
  wi_number <- grab_wi_number(one_url)
  if(is.na(wi_number)){
  ...
  viaf_number <- read_html(paste0(base_url, wi_number))%>%
...
t <- read_html("http://classify.oclc.org/classify2/ClassifyDemo?wi=326454")%>%
  html_nodes("#display-V-tbl a , #display-Summary a")%>%
  # html_nodes("#display-V-tbl a")%>%
  html_text()

t2 <- read_html("http://classify.oclc.org/classify2/ClassifyDemo?wi=326454")%>%
  html_nodes("dd:nth-child(6) a:nth-child(1)")%>%
  html_text()
