...
Once the model is trained, I can generate predictions for beers a user has not reviewed yet. Here I am just selecting a random user(but the functionality is there to specify one) and then building a matrix if that user and all distinct beers. I feed that into recosystems predict function and it will generate a prediction for that user for each beer. Since some of those beers have already been reviewed by the user, I filter those out(and those could be used to measure how well the model is doing later) and then return the top 3 rated beers for the user. The helper functions in the code below allow me to generate predictions for a user and then quickly pull up the top beers for that user and gauge how good the models predictions were.

```{r, recommendation-and-helper-functions}
top_three_beers_for_user <- function(reco_object = r, user_beer_sparse = user_beer_triplet, user_id = NULL){
# inputs: 
#       reco_object - the r object recosystem created in training the model
#       user_beer_sparse - triplet matrix used to build prediction matrix
#       user_id - specific user to recommend beers for; defaults to randomly selected user
  ...
  # generate predictions and add to prediction grid
...
users_top_rated_beers <- function(df = over_50_reviews, user = NULL){
  ...
  df%>%
    filter(user_id == user)%>%
    select(user_id, beer_id, beer_name, brewery_name, beer_style, review_overall)%>%
...
Picking two random users from the dataset you can see the different predictions for each user.
