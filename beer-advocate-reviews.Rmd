...
temp <- beer_style_conf_int%>%
  top_n(15, estimate)

temp2 <- beer_style_conf_int%>%
  top_n(-15, estimate)
...
temp3%>%
  ...
  geom_errorbarh(aes(xmin = conf.low,
                     xmax = conf.high))+
  geom_hline(yintercept = 15.5, linetype = 'dashed')+
...
This linear model is predicting review_overall from the sub-reviews--taste, aroma, appearance and palate. With an R-squared of just .658, this model isn't super accurate, but it's good enough for our purposes.[^2] I just want to see which factor is most important to the overall review score after all. It's tempting to just look at the coefficient values and say whichever is larger is more important, but that's [not really true](https://stats.stackexchange.com/a/202846). Variables on different scales, variables with wildly different variances or multi-collinearity between variables will all make determining feature importance from the coefficient values problematic(if not outright wrong).

[^2]: I did explore including interaction effects in the model, but they didn't provide much improvement and they make determining feature importance a little more difficult. Ultimately I decided to ignore them.
...
As usual, the full code for this report can be found [here](https://github.com/Grollus/DataBlog/tree/master/content/post) and thanks for reading!
